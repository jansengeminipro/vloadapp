        ...
        <div className="relative" ref={ref}>
            ...
            {isOpen && (
                <>
                    ...
                    <div className="absolute top-full right-0 mt-2 md:mt-1 md:left-auto md:right-0 fixed md:absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 md:translate-x-0 md:translate-y-0 w-[90%] md:w-64 max-w-sm bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-50 p-4 animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="max-h-64 overflow-y-auto custom-scrollbar space-y-1">
                            {activeMuscleGroups
                                ...
                                .filter(m => MUSCLE_COLORS[m])
                                .map(m => (
                                    <button
                                        key={m}
                                        onClick={() => toggleMuscle(m)}
                                        className="w-full flex justify-between items-center px-3 py-2 text-xs text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-colors border border-transparent hover:border-slate-700"
                                    >
